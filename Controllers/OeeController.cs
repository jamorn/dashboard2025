...
public class OEEController : ControllerBase
{
    ...
    public async Task<ActionResult<MachineOEEData>> GetMachineOEEData()
    {
        ...
        {
            var oeeData = await _context.Dashboards
                .Include(d => d.Machine)
                .Include(d => d.RemarkItems) // เพิ่ม Include RemarkItems
                .Where(d => d.Machine.MachineName == machine.MachineName 
                       && d.RecordDate >= startDate 
                       && d.RecordDate <= latestDate)
                .Join(
                    _context.KPI,
                    oee => oee.Machine.UnitId,
                    kpi => kpi.UnitId,
            ...
            var existingDates = oeeDataDTOs
                ...
                .Select(o => DateTime.ParseExact(o.DateString!, "yyyy-MM-dd", CultureInfo.InvariantCulture)) // ใช้ ! ในการกำหนดว่า DateString ไม่ใช่ null
                .ToList();
            ...
            foreach (var date in missingDates)
            {
                ...
                {
                    Item = oeeDataDTOs.Count() + 1,  // เปลี่ยนเป็นใช้ Count() + 1 เพื่อให้ต่อจากลำดับล่าสุด
                    MachineName = machine.MachineName, // ใช้ MachineName ของเครื่องจักรปัจจุบัน
                    Availability = 0,
                    Performance = 0,
                    Quality = 0,
                    OEE = 0,
                    Giveaway = 0,
                    Remarks = new[] { "เครื่องจักรหยุดทำงาน" }, // กำหนดเป็น array ที่มี string เดียว
